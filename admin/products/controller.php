...
function doInsert(){
	...
	if(isset($_POST['save'])){
		// Trim the product description to remove leading and trailing whitespace
		$productDesc = trim($_POST['PRODESC']);
	
		// Retrieve the existing product information
		$product = New Product();
		$existing_product = $product->single_product($_POST['PROID']);
	
		// Calculate the new quantity by adding the existing quantity and the quantity from the form
		$new_quantity = $existing_product->PROQTY + $_POST['PROQTY'];
	
		// Update the product with the new quantity and trimmed product description
		$product->PRODESC = $productDesc;
		$product->CATEGID = $_POST['CATEGORY'];
		$product->PROQTY = $new_quantity; // Update with the new quantity
		$product->ORIGINALPRICE = $_POST['ORIGINALPRICE'];
		$product->PROPRICE = $_POST['PROPRICE'];
